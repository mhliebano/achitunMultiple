...
function fGuardar($tabla=null,$accion=0,$msg=true){
    ...
    $cantCampos=0;
    $cantDatos=0;
    if (count(fEnviadoPost())<1 && $accion==0){
        return;
    }
switch($accion){
    case 0:
...
function fVerificaFunciones($archivo,$nombre){
    ...
    switch($archivo){
        case 1:
            ...
            break;
        case 2:
            $content = file_get_contents("modelos/M$nombre.php");
            preg_match_all("/(function )(\S*\(\S*\))/", $content, $matches);
            foreach($matches[2] as $match) {
                $function[] = trim($match);
            }
            ...
            break;
        case 3:
            $content = file_get_contents("vistas/V$nombre.php");
            preg_match_all("/(function )(\S*\(\S*\))/", $content, $matches);
            foreach($matches[2] as $match) {
                $function[] = trim($match);
            }
            ...
            break;
        case 4:
            $content = file_get_contents("controles/C$nombre.php");
            preg_match_all("/(function )(\S*\(\S*\))/", $content, $matches);
            foreach($matches[2] as $match) {
                $function[] = trim($match);
            }
